...
import { AddBloque, AddContenido } from "./ModalesCursosCrear";
...
export function InterfazSubirArchivo({ ContenidoID }: { ContenidoID: number }) {
  ...
  const Default = "Por defecto";
  const DefaultSelect = [{ ID: 1, nombre: "cargando..." }];
    ...
    mutationFn: async () => {
      ...
      if ((TXT_URL == null || TXT_URL == "") && selectedFile == null) {
        ...
        const { data } = await axios({
          method: "post",
          baseURL: Dominio,
          url: "/archivo/subir/recursoToCurso",
          data: formData,
          params: { ContenidoID: ContenidoID },
  ...
  // if (
  //   FetchIdioma == undefined ||
  //   FetchNivelAgregacion == undefined ||
  //   FetchNivelInteractividad == undefined ||
  //   FetchTipoRecursoEducativo == undefined ||
    ...
    <Container component={"main"} maxWidth="md">
      ...
      >
        ...
        <Autocomplete
          ...
          //defaultValue={FetchTipoConocimiento[0]|| 0}
          getOptionLabel={(option: any) => option.Nombre.toString()}
          isOptionEqualToValue={(option, value) => option.ID === value.ID}
          sx={{ width: "15rem", paddingInline: "0.5rem" }}
          onChange={(event, value: any) => {
...
function NullSelectParser<Type>(Objeto?: Type[]) {
  if (Objeto != undefined) {
    return Objeto[0];
  } else {
    return 0;
