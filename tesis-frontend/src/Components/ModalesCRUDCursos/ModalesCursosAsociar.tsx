...
function RenderAsociarAMateria({ ContenidoID }: { ContenidoID: number }) {
  const {
    data,
  ...
  const {
    ...
    const { data } = await axios({
      baseURL: Dominio,
      method: "get",
      url: "/recursos/MaterialesContenidoCurso",
      params: {
        ContenidoID: ContenidoID,
      },
    });
  ...
  const Asociar = useMutation({
    mutationFn: async () => {
      const { data } = await axios({
        baseURL: Dominio,
        method: "put",
        url: `/recursos/ascociar/materialCurso/${ContenidoID}`,
        data: {
...
export function LinkMaterialToCurso({
  children,
  ContenidoID,
}: {
  ContenidoID: number;
