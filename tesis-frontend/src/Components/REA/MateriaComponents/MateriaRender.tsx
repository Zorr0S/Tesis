...
//                 Curso
//                 {/* {BloqueVista} */}
//                 <BloqueRender Data={data.Bloques} />
//               </Grid>
//             </Grid>
//           </Container>
//         ))}
...
//   }
  ...
  export function MateriaBloqueRender({ Data }: { Data: Bloque[] }) {
    return (
      <>
        {Data.map((data, index) => (
          <>
            <Accordion key={`Acordeon + ${data.Nombre} + ${data.ID}-${index}`} expanded>
              ...
              >
                <Typography variant="h4"  key={`${data.ID}-Bloque-nomb`}>{data.Nombre}</Typography>
              </AccordionSummary>
              <AccordionDetails key={`AcordeonDet + ${data.Nombre} + ${data.ID}-${index}`}>
                <Typography  key={`${data.ID}-Bloque-desc`}>{data.Descripcion}</Typography>
              </AccordionDetails>
              <MateriaContenidoRender Data={data.Contenido} key={`${data.ID}-Bloque-Conte`} />
            </Accordion>
          </>
        ))}
      </>
  ...
  export function MateriaContenidoRender({ Data }: { Data: ContenidoBloque[] }) {
    return (
      <>
        {Data.map((post) => (
          <List>
   ...
   function Cortador({
    ...
    const { data, isError, isFetching } = useQuery(
      `Contenido-General-${MaterialID}`,
      async () => {
        const { data } = await axios({
          baseURL: Dominio,
          method: "get",
          url: "/recursos/MaterialesLista",
          params: {
    ...
    return (
      <>
        <MateriaMaterialesRender Data={data} />
        {/* {Expansor} */}
      </>
    );
  }
  
  export function MateriaMaterialesRender({ Data }: { Data: Recurso[] }) {
